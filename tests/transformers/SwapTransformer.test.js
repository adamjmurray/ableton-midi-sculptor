...
describe('SwapTransformer', function () {
    var notes;
    var swapTransformer;
    beforeEach(function () { return swapTransformer = new SwapTransformer(); });
    function makeNotes() {
        var values = [];
    ...
    describe('rotate()', function () {
        beforeEach(function () { return useNotes(1, 2, 3, 4); });
        it('rotates forward through the note list when given a positive percentage value', function () {
        ...
        it('rotate backwards through the note list when given a negative percentage value', function () {
            var expected = clone(notes).slice(1).concat(notes.slice(0, 1));
            // rotating notes changes everything except the start time:
            expected.forEach(function (note, index) { return note.start = notes[index].start; });
            assert.deepEqual(swapTransformer.rotate(-1 / notes.length), expected);
        ...
        it('can rotate more than one position', function () {
        ...
        it('can rotate with a value > notes.length and wraps around', function () {
            var expected = clone(notes).slice(-3).concat(notes.slice(0, -3));
            // rotating notes changes everything except the start time:
            expected.forEach(function (note, index) { return note.start = notes[index].start; });
